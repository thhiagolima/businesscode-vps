...
domains="api.businesscode.com.br dev.businesscode.com.br"
...
for d in api.businesscode.com.br dev.businesscode.com.br; do
...
# api e dashboard em certs separados
docker compose run --rm --entrypoint "\
...
docker compose run --rm --entrypoint "\
  certbot certonly --webroot -w /var/www/certbot $staging_arg \
    --email $email --agree-tos --no-eff-email \
    -d dev.businesscode.com.br" certbot
